...
The hypotheses will be tested using data from several sources. The first source of data comes from energy company Westland Infra, who have publicly available yearly data about energy usage for postal code areas of different sizes in the Dutch municipalities of Westland and Midden-Delfland (<https://westlandinfra.nl/over-westland-infra/duurzaamheid-innovaties/open-data/>). From this data we will use our dependent variable, which is the percentage of electricity connections in a postal code area that have a net negative electricity usage over the year. As one needs to produce electricity to end up with a net negative electricity usage, a higher percentage represents the presence of more solar panel installations. Research on the Dutch island of Texel shows that around 17% of households with solar panels are yearly net producers (Aydın et al., 2023).
...
pv <- readRDS("./data/pv_data.Rds") |>
  ...
  mutate(area_id = as.integer(factor(paste(postcode_van, postcode_tot, sep = "_")))) |>
  select(id, area_id, postcode_van, postcode_tot, straatnaam, year, leveringsrichting)
...
Netherland for each (PC6) postal code (<https://www.cbs.nl/nl-nl/dossier/nederland-regionaal/geografische-data/gegevens-per-postcode>). The data ranges from 2015 to 2024. After the 1st of May the data of 2019 and 2020 should be available as well!
...
The postal code areas in the data from Westland Infra can span multiple PC6 postal codes. First we select all of the PC6 postal codes in the CBS data that are contained in the postal code ranges of the Westland Infra data.
...
Next we aggregate this data from single postal codes of the PC6 data to the postal code areas of the Westland Infra data.
...
Some data, such as density and income, is only available at the level of 500m by 500m squares ([https://www.cbs.nl/nl-nl/dossier/nederland-regionaal/geografische-data/kaart-van-500-meter-bij-500-meter-met-statistieken](#0){.uri}).

We also use these squares to calculate the demographic diversity of the surrounding neighborhood. These will also be the areas used to calculate the effect of neighborhood solar panels.
...
df_cbs <- df_cbs |> arrange(area_id, year)
...
impute_ema <- function(df, target_year, history_years, var) {
  imputed <- df |>
    ...
    sapply(function(pc) {
      hist_values <- df |>
        filter(area_id == pc, year %in% history_years) |>
        arrange(year) |>
        pull({{ var }}) |>
        na.omit()
    ...
    enframe(name = "area_id", value = "imputed_value")
  
  df |>
    left_join(imputed, by = "area_id") |>
...
There are still many other missing values, so we impute (some) missing values using spatial interpolation, by taking the mean of neighboring postal code areas.

```{r}

df_cbs <- df_cbs |> arrange(area_id, year)
areas <- df_cbs |> distinct(area_id, .keep_all = TRUE)
nb <- poly2nb(areas, queen = TRUE)

  years <- c(2016:2019, 2022:2024)
  area_ids  <- areas$area_id
  
spatial_impute <- function(df, var) {
  print(summary(df[[var]]))
  
  for (yr in years) {
    yr_data <- df[df$year == yr, ] |> arrange(area_id)
    vals    <- yr_data[[var]]
    ...
    for (i in missing) {
      ...
      if (length(nbr_vals) > 0) {
        df[df$year == yr & df$area_id == area_ids[i], var] <- mean(nbr_vals)
      }
    }
  }
...
Calculating demographic diversity in the 500m x 500m squares using the inverse Herfindahl index.

```{r, eval=T}
##economic diversity

#first impute missing values for income.
df_cbs <- spatial_impute(df_cbs, "percentage_hoog_inkomen_huishouden")
df_cbs <- spatial_impute(df_cbs, "percentage_laag_inkomen_huishouden")
...
#first impute missing values for ethnicity.
df_cbs <- spatial_impute(df_cbs, "percentage_geb_nederland_herkomst_nederland_500")
...
#impute missings
df_cbs <- spatial_impute(df_cbs, "percentage_inwoners_0_tot_15_jaar_500")
df_cbs <- spatial_impute(df_cbs, "percentage_inwoners_15_tot_25_jaar_500")
df_cbs <- spatial_impute(df_cbs, "percentage_inwoners_25_tot_45_jaar_500")
df_cbs <- spatial_impute(df_cbs, "percentage_inwoners_45_tot_65_jaar_500")
df_cbs <- spatial_impute(df_cbs, "percentage_inwoners_65_jaar_en_ouder_500")
...
df_cbs <- spatial_impute(df_cbs, "gemiddeld_elektriciteitsverbruik_woning")
...
df_cbs <- spatial_impute(df_cbs, "percentage_koopwoningen")

## ethnicity
summary(df_cbs$percentage_geb_nederland_herkomst_nederland)

df_cbs <- spatial_impute(df_cbs, "percentage_geb_nederland_herkomst_nederland")
...
df |>
  filter(year == 2024) |>
  select(area_id, crs28992res500m, nb_pv, year) |>
  left_join(
    avg_500 |> filter(year == 2023) |> rename(nb_pv_2023 = nb_pv),
    by = "crs28992res500m"
  ) |>
  mutate(match = near(nb_pv, nb_pv_2023)) |>
  count(match)
